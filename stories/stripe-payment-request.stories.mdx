...
# Using `<stripe-payment-request>` Across Frameworks

Since `<stripe-payment-request>` is a custom-element, you can easily use it across frameworks.

## Using with Plain HTML and JavaScript

<Story name="In Plain HTML and JavaScript">
...
class PaymentForm extends LitElement {
  ...
  render() {
    return html`
      ...
      <json-viewer .object="${ifDefined(this.paymentMethod)}"></json-viewer>
    `;
  }
